...
---------+---------+---------+---------+---------X---------+---------+---------+---------+---------X
$Lastupdate: 2021/11/11 11:15:03 $ T.AIHANA
```
...
記号 ``:`` を使って変数の型を明示的に指定できます（この記号は「型の注釈（*annotation*）」を表します）：
...
次の例で、`std` は *crate* （と言うライブラリ）、`cmp` はモジュール名（ソースファイル）、そして `min` は関数名とすると：

```Rust
let least = std::cmp::min(3, 8);    // これは 3
```

命令 `use` は他の名前空間からいくつかの名前を「スコープの中に取り込む」際に使用します：
