...
---------+---------+---------+---------+---------X---------+---------+---------+---------+---------X
$Lastupdate: 2021/11/09 10:38:49 $ T.AIHANA
```
...
記号 ``:`` を使って変数の型を明示的に指定できます（この記号は「型の注釈」を表します）：
...
も変数 `pair` の型をきちんと注釈を付けたいのであれば、次のように書きます：
...
もちろんタプルを分解する際に `_` を使って一部の要素を破棄することもできます：
